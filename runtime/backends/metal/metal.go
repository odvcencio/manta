...
func (b *Backend) CanLoad(mod *barr.Module) bool {
	return mod != nil && mod.SupportsBackend(barr.BackendMetal)
}
...
func (b *Backend) load(_ context.Context, mod *barr.Module, weights map[string]backend.WeightBinding, cacheKey string) (backend.Executor, error) {
	...
	compiled, err := backend.CompileVariants(mod, barr.BackendMetal)
	if err != nil {
		return nil, err
	}
	...
	for _, kernel := range mod.Kernels {
		prog, err := backend.CompileNativeKernelProgram(barr.BackendMetal, kernel, compiled[kernel.Name])
		if err != nil {
			if device != nil {
				device.close()
			}
...
func (e *executor) Run(ctx context.Context, req backend.Request) (backend.Result, error) {
	return backend.ExecuteSymbolic(ctx, e.module, e.weights, e.compiled, e.dispatchKernel, e.dispatchStep, barr.BackendMetal, req)
}
